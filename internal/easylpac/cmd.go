...
import (
	...
	"EasyLPAC/internal/easylpac/platform"
	"bufio"
	"bytes"
	"encoding/json"
	"errors"
...
func runLpac(args ...string) (json.RawMessage, error) {
	...
	// Save to LogFile
	lpacPath := filepath.Join(appconfig.Instance.LpacDir, appconfig.Instance.EXEName)
	command := lpacPath
	for _, arg := range args {
		command += fmt.Sprintf(" %s", arg)
	}
	if _, err := fmt.Fprintln(appconfig.Instance.LogFile, command); err != nil {
		return nil, err
	}
	...
	cmd.Dir = appconfig.Instance.LpacDir

	cmd.Env = []string{
		"LPAC_APDU=pcsc",
		"LPAC_HTTP=curl",
		fmt.Sprintf("DRIVER_IFID=%s", appconfig.Instance.DriverIFID),
		fmt.Sprintf("LPAC_CUSTOM_ISD_R_AID=%s", appconfig.Instance.LpacAID),
	}
	if appconfig.Instance.DebugHTTP {
		cmd.Env = append(cmd.Env, "LIBEUICC_DEBUG_HTTP=1")
	}
	if appconfig.Instance.DebugAPDU {
		cmd.Env = append(cmd.Env, "LIBEUICC_DEBUG_APDU=1")
	}
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	writer := io.MultiWriter(&stdout, appconfig.Instance.LogFile)
	errWriter := io.MultiWriter(appconfig.Instance.LogFile, &stderr)
	cmd.Stdout = writer
	...
	var resp model.LpacReturnValue
...
func LpacChipInfo() (*model.EuiccInfo, error) {
	payload, err := runLpac("chip", "info")
	if err != nil {
		return nil, err
	}
	var chipInfo *model.EuiccInfo
...
func LpacProfileList() ([]*model.Profile, error) {
...
func LpacProfileDownload(info model.PullInfo) {
	...
	if err != nil {
		...
		Refresh()
		downloadNotification := findNewNotification(notificationOrigin, model.Notifications)
...
func LpacNotificationList() ([]*model.Notification, error) {
...
func LpacDriverApduList() ([]*model.ApduDriver, error) {
	payload, err := runLpac("driver", "apdu", "list")
	if err != nil {
		return nil, err
	}
	var apduDrivers []*model.ApduDriver
